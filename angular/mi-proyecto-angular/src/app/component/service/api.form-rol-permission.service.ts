...
export class ApiFormRolPermissionService {
  ...
  getData(): Observable<any> {
    ...
    return this.https.get<any>('/api/FormRolPermissions'); // 👈 Aquí corriges
  }

  // GET: Obtiene un elemento por su ID
  getById(id: number): Observable<any> {
    return this.https.get<any>(`/api/FormRolPermissions/${id}`); // 👈 Corrige aquí también
  }

  // POST: Crea un nuevo elemento
  postData(data: any): Observable<any> {
    return this.https.post<any>('/api/FormRolPermissions', data); // 👈 Corrige aquí
  }

  // PUT: Actualiza un elemento
  putData(id: number, data: any): Observable<any> {
    return this.https.put<any>(`/api/FormRolPermissions/${id}`, data); // 👈 Corrige aquí
  }

  // DELETE: Elimina un elemento
  deleteData(id: number): Observable<any> {
    return this.https.delete<any>(`/api/FormRolPermissions/${id}`); // 👈 Corrige aquí
  }
